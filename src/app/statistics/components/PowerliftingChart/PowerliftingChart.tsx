...
export default function PowerliftingChart({
  data
}: {
  data: StatisticsDto[]
}) {
  return (
    <BarChart data={data} width={960} height={320} margin={{ bottom: 48 }} barGap={4} barCategoryGap={16}>
      ...
      <Bar dataKey="backSquat" fill="#E94F37" name="Zadní dřep (kg)" />
      <Bar dataKey="benchPress" fill="#EFA00B" name="Bench press (kg)" />
      <Bar dataKey="deadlift" fill="#6BAA75" name="Mrtvý tah (kg)" />
    </BarChart>
  );
}
